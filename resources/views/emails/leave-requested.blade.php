@extends('emails.layout')
@php
  $headerTitle    = "New Leave Request Submitted";
  $headerSubtitle = "{$leave->employee->full_name} is requesting time off";
  $ctaUrl         = $adminUrl;
  $ctaText        = "Review Request";
@endphp
@section('body')

<p>Hi there,</p>
<p>A new leave request requires your review. Here are the details:</p>

{{-- Info Card --}}
<table width="100%" cellpadding="0" cellspacing="0" style="background:#F8F9FF; border-radius:12px; border:1px solid #E0E7FF; margin:20px 0; overflow:hidden;">
  <tr>
    <td style="background: linear-gradient(135deg,#4F46E5,#7C3AED); padding:14px 24px;">
      <span style="color:#fff; font-weight:700; font-size:15px;">Leave Request Details</span>
    </td>
  </tr>
  <tr>
    <td style="padding:20px 24px;">
      <table width="100%" cellpadding="0" cellspacing="0">
        @foreach([
          ['Employee',   $leave->employee->full_name],
          ['Leave Type', $leave->leaveType->name ?? '-'],
          ['From',       $leave->start_date->format('D, M j, Y')],
          ['To',         $leave->end_date->format('D, M j, Y')],
          ['Days',       ($leave->total_days ?? '-') . ' working day(s)'],
          ['Status',     'Pending Approval'],
        ] as [$label, $value])
        <tr>
          <td style="padding:7px 0; width:130px; font-size:13px; color:#9CA3AF; font-weight:500;">{{ $label }}</td>
          <td style="padding:7px 0; font-size:14px; color:#111827; font-weight:600;">{{ $value }}</td>
        </tr>
        @endforeach
      </table>
    </td>
  </tr>
  @if($leave->reason)
  <tr>
    <td style="padding:0 24px 20px;">
      <div style="background:#fff; border-radius:8px; border:1px solid #E5E7EB; padding:14px 16px;">
        <div style="font-size:11px; color:#9CA3AF; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:6px;">Reason</div>
        <div style="font-size:14px; color:#374151; line-height:1.6;">{{ $leave->reason }}</div>
      </div>
    </td>
  </tr>
  @endif
</table>

<p style="color:#6B7280; font-size:14px;">Please review and approve or reject this request in the admin portal.</p>
<p style="color:#374151; font-size:14px;">— <strong>{{ $companyName }} HR System</strong></p>

@endsection
