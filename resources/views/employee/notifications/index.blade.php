...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-2">
            <form action="{{ route('employee.notifications.read-all', $tenant) }}" method="POST">
                @csrf
            ...
            <form action="{{ route('employee.notifications.clear-all', $tenant) }}" method="POST"
    ...
    <div class="lmt-card p-0 divide-y divide-gray-100 dark:divide-slate-700">
        ...
        <div class="flex items-start gap-4 px-5 py-4 hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                @if($notif->isUnread())
                <form action="{{ route('employee.notifications.read', [$tenant, $notif->id]) }}" method="POST">
                    @csrf @method('PATCH')
                ...
                @endif
                <form action="{{ route('employee.notifications.destroy', [$tenant, $notif->id]) }}" method="POST">
                    @csrf @method('DELETE')
