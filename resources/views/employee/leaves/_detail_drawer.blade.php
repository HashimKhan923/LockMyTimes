...
<div class="p-6 pt-12">
    ...
    <div class="flex items-start gap-3 mb-6">
        ...
        <div class="flex-1 min-w-0">
            <p class="font-mono text-xs font-bold text-gray-400">{{ $lr->request_number }}</p>
    ...
    <div class="p-4 rounded-2xl mb-4" style="background:var(--brand-50);">
        ...
        <div class="flex items-center gap-3">
            <div class="text-center">
                <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wide">{{ $start->format('M') }}</p>
                <p class="text-2xl font-black font-mono mt-0.5" style="color:var(--brand-700);">{{ $start->format('j') }}</p>
                <p class="text-[10px] text-gray-400 mt-0.5">{{ $start->format('D, Y') }}</p>
            </div>
            <i data-lucide="arrow-right" class="w-5 h-5 mx-3" style="color:var(--brand-500);"></i>
            <div class="text-center">
                <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wide">{{ $end->format('M') }}</p>
                <p class="text-2xl font-black font-mono mt-0.5" style="color:var(--brand-700);">{{ $end->format('j') }}</p>
                <p class="text-[10px] text-gray-400 mt-0.5">{{ $end->format('D, Y') }}</p>
            </div>
            <div class="ml-auto text-right">
                <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wide">Total</p>
                <p class="text-2xl font-black font-mono mt-0.5" style="color:var(--brand-700);">
                    {{ rtrim(rtrim(number_format($lr->total_days, 1), '0'), '.') }}
                </p>
                <p class="text-[10px] text-gray-400 mt-0.5">day{{ $lr->total_days == 1 ? '' : 's' }}</p>
            </div>
        </div>
        @if($lr->day_part && $lr->day_part !== 'full')
            <p class="text-xs text-gray-500 font-semibold mt-3 capitalize">
    ...
    {{-- Reason --}}
    <div class="mb-4">
        <p class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Reason</p>
    ...
    @if($lr->contact_during_leave)
        <div class="mb-4 p-4 rounded-2xl border border-gray-100 dark:border-slate-700">
            <div class="flex items-center gap-2 mb-1">
                ...
                <p class="text-xs font-bold uppercase tracking-wider text-gray-500">Contact during leave</p>
            </div>
    ...
    @if(is_array($lr->attachments) && count($lr->attachments) > 0)
        <div class="mb-4">
            <p class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Attachments</p>
            <div class="space-y-2">
                @foreach($lr->attachments as $att)
                    <div class="flex items-center gap-3 p-3 rounded-xl border border-gray-100 dark:border-slate-700">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[10px] text-gray-400">{{ isset($att['size']) ? round($att['size']/1024).' KB' : '' }}</p>
                        </div>
                        @if(isset($att['path']))
    ...
    {{-- Approval timeline --}}
    <div class="mt-6">
        <p class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-3">Timeline</p>
        <div class="space-y-3">
            ...
            <div class="flex gap-3">
                ...
                <div class="flex-1 pb-1">
                    ...
                    <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($lr->created_at)->format('M j, Y · h:i A') }}</p>
                </div>
            </div>
            ...
            @if($lr->status === 'approved')
                <div class="flex gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        @if($lr->approver)
                            <p class="text-xs text-gray-500">by {{ $lr->approver->name }}</p>
                        @endif
                        @if($lr->approved_at)
                            <p class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($lr->approved_at)->format('M j, Y · h:i A') }}</p>
                        @endif
            ...
            @elseif($lr->status === 'cancelled')
                <div class="flex gap-3">
                    <div class="w-7 h-7 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center flex-shrink-0">
                        <i data-lucide="ban" class="w-3.5 h-3.5"></i>
                    </div>
                    <div class="flex-1">
                        <p class="text-sm font-bold text-gray-900 dark:text-slate-100">Cancelled</p>
                        <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($lr->updated_at)->format('M j, Y · h:i A') }}</p>
                    </div>
                </div>
            @elseif($lr->status === 'pending')
                <div class="flex gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-gray-500">Your manager will be notified.</p>
                    </div>
                </div>
            @endif
