            ...
            <div class="lmt-card p-0 overflow-hidden" data-lmt-anim="fade-up">
                <div class="flex items-center justify-between p-5 border-b border-gray-100 dark:border-slate-700">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">{{ $myTasks->count() }} task{{ $myTasks->count() == 1 ? '' : 's' }} assigned to you</p>
                    </div>
                </div>
                ...
                @if($myTasks->isEmpty())
                    <div class="text-center py-16 px-5">
                        ...
                        <p class="text-xs text-gray-500 mt-1">You're a member of this project but have no tasks assigned yet.</p>
                    </div>
                @else
                                        ...
                                        <div class="flex items-center gap-2 flex-wrap">
                                            ...
                                            @if($t->priority !== 'normal')
                                                <span class="text-[9px] font-bold px-1.5 py-0.5 rounded uppercase
                                                    ...
                                                    {{ $t->priority === 'low' ? 'bg-gray-50 text-gray-500 dark:bg-slate-700' : '' }}">
                                                    {{ $t->priority }}
                                                </span>
                                            @endif
                                            @if($t->taskList)
                                                <span class="text-[10px] text-gray-400">{{ $t->taskList->name }}</span>
                                            @endif
                                        </div>
            ...
            @if($members->isNotEmpty())
                <div class="lmt-card" data-lmt-anim="fade-up">
                    <h3 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                        ...
                        <span class="font-mono text-[10px] text-gray-300 ml-1">({{ $members->count() }})</span>
                    </h3>
                    <div class="space-y-2">
                            ...
                            @if($m->employee)
                                <div class="flex items-center gap-2.5 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-[10px] text-gray-400 font-bold uppercase">{{ str_replace('_', ' ', $m->role) }}</p>
                                    </div>
                                </div>
                            @endif
                        @endforeach
                        @if($members->count() > 10)
                            <p class="text-[10px] text-gray-400 text-center mt-2">+ {{ $members->count() - 10 }} more</p>
                        @endif
                    </div>
                </div>
            @endif
