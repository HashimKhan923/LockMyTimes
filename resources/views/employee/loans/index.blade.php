...
<div x-data="{ tab: '{{ $tab }}' }">
    ...
    <div class="flex items-center gap-1 mb-5 border-b border-gray-200 dark:border-slate-700">
        <button @click="tab='loans'"
                :class="tab==='loans' ? 'text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-slate-300'"
            ...
            <span class="ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold"
                  :class="tab==='loans' ? 'bg-white/20 text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-500'">
                {{ (int) $loanStats->total }}
            </span>
        </button>
        <button @click="tab='advances'"
                :class="tab==='advances' ? 'text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-slate-300'"
                class="px-4 py-2.5 text-sm font-bold rounded-t-xl -mb-px transition-all relative"
                :style="tab==='advances' ? 'background:var(--brand-500);' : ''">
            <i data-lucide="zap" class="w-4 h-4 inline -mt-0.5"></i>
            Salary Advances
            <span class="ml-1.5 px-1.5 py-0.5 rounded-full text-[10px] font-bold"
                  :class="tab==='advances' ? 'bg-white/20 text-white' : 'bg-gray-100 dark:bg-slate-700 text-gray-500'">
            ...
            @foreach($statCards as $card)
                ...
                <div class="lmt-card p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{{ $card['label'] }}</p>
        ...
        <div class="lmt-card p-0 overflow-hidden" data-lmt-anim="fade-up">
            <div class="flex items-center justify-between p-5 border-b border-gray-100 dark:border-slate-700">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">All your loan applications and active loans</p>
                </div>
            ...
            @if($loans->isEmpty())
                <div class="text-center py-16 px-5">
                    <div class="w-16 h-16 mx-auto rounded-2xl bg-gray-50 dark:bg-slate-800 flex items-center justify-center mb-3">
                        <i data-lucide="hand-coins" class="w-7 h-7 text-gray-300"></i>
                    </div>
                    <p class="text-sm font-bold text-gray-700 dark:text-slate-200">No loans yet</p>
                    <p class="text-xs text-gray-500 mt-1">Apply for your first loan to see it here.</p>
                            ...
                            @foreach($loans as $ln)
                                ...
                                <tr class="hover:bg-gray-50/70 dark:hover:bg-slate-800/50">
                                    <td>
                                        ...
                                        <p class="text-[10px] text-gray-400 mt-0.5">{{ $ln->created_at->format('M j, Y') }}</p>
                                    </td>
                                    <td>
                                        <span class="text-sm font-bold text-gray-900 dark:text-slate-100">{{ $ln->loanType?->name ?? '—' }}</span>
                                        @if($ln->tenure_months)
                                            <p class="text-[10px] text-gray-400">{{ $ln->tenure_months }} months @ {{ number_format($ln->interest_rate, 2) }}%</p>
                                        @endif
                                    </td>
                                    <td class="text-right">
                                    ...
                                    <td class="hidden lg:table-cell" style="min-width:160px;">
                                        @if(in_array($ln->status, ['disbursed', 'active', 'completed']))
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-[10px] font-bold text-gray-500 flex-shrink-0">{{ $lnProgress }}%</span>
                                            </div>
                                            <p class="text-[10px] text-gray-400 mt-1">{{ $ln->installments_paid }}/{{ $ln->tenure_months }} paid</p>
                                        @else
        ...
        <div class="lmt-card p-0 overflow-hidden" data-lmt-anim="fade-up">
            <div class="flex items-center justify-between p-5 border-b border-gray-100 dark:border-slate-700">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-0.5">Quick advances repaid through payroll deductions</p>
                </div>
            ...
            @if($advances->isEmpty())
                <div class="text-center py-16 px-5">
                    <div class="w-16 h-16 mx-auto rounded-2xl bg-gray-50 dark:bg-slate-800 flex items-center justify-center mb-3">
                        <i data-lucide="zap" class="w-7 h-7 text-gray-300"></i>
                    </div>
                    <p class="text-sm font-bold text-gray-700 dark:text-slate-200">No advances yet</p>
                    <p class="text-xs text-gray-500 mt-1">Request your first salary advance to see it here.</p>
                            ...
                            @foreach($advances as $adv)
                                ...
                                <tr class="hover:bg-gray-50/70 dark:hover:bg-slate-800/50">
                                    <td>
                                        ...
                                        <p class="text-[10px] text-gray-400 mt-0.5">{{ $adv->created_at->format('M j, Y') }}</p>
                                    </td>
                                    <td class="text-right">
                                        <span class="font-mono font-bold text-sm text-gray-900 dark:text-slate-100">
                                    ...
                                    <td class="hidden lg:table-cell" style="min-width:160px;">
                                        @if(in_array($adv->status, ['active', 'disbursed', 'completed']))
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span class="text-[10px] font-bold text-gray-500 flex-shrink-0">{{ $advProgress }}%</span>
                                            </div>
                                            <p class="text-[10px] text-gray-400 mt-1">{{ $adv->installments_paid }}/{{ $adv->installments_count }} paid</p>
                                        @else
                                            <span class="text-xs text-gray-400">—</span>
                                        @endif
                                    </td>
